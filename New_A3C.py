        ...
        with sess.as_default(), sess.graph.as_default():
            while episode_count < max_training_episodes:  # not coord.should_stop():
                ...
                while (d == False and episode_step_count < max_episode_length - 1):  # modified to continue looping
                    ...
                    #if d == False:
                    if self.bool_evaluating == True:
                        r,s1,_ = new_transition(s, actions[a], Demand[episode_step_count], LT_s, LT_f, h, b, C_s, C_f, InvMax, InvMin)
                        d = False
                        #reward_state = deepcopy(self.update_state(Demand[episode_step_count], actions[a]))
                        #s1 = deepcopy(self.Transition())
                    else:
                        r, s1, d = new_transition(s, actions[a], random.randint(0, Demand_Max), LT_s, LT_f, h, b, C_s,
                                                  C_f, InvMax, InvMin)
                        #reward_state = deepcopy(self.update_state(random.randint(0, Demand_Max), actions[a]))
                        #s1 = deepcopy(self.Transition())
                    #r = self.Reward(reward_state, actions[a])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_no_improvement', default=5000, type=float, help="max_no_improvement. Default = 5000", dest="max_no_improvement")
    ...
    parser.add_argument('--depth_nn_layers_hidden', default=[40,20,10,10], type=float,
